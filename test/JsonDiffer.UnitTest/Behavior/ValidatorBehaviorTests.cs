...
namespace JsonDiffer.UnitTest.Behavior
{
    public class ValidatorBehaviorTests
    {
        ...
        [Fact]
        public void Should_run_next_when_there_is_no_validatior()
        {
            _validators = new IValidator<object>[0];
            var behavior = new ValidatorBehavior<object,object>(_validators);
            behavior.Handle(new object(), default(CancellationToken), NextHandle);
            Assert.True(_delegateHaveBeenCalled);
        }

        [Fact]
        public void Should_run_next_when_everything_is_valid()
        {
